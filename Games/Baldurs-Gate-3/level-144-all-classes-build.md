...
1. Запустить чистую игру хотя бы один раз и проверить в главном меню версию `4.1.1.7209685`.
2. Установить актуальный [BG3 Mod Manager](https://github.com/LaughingLeader/BG3ModManager). В GOG Galaxy путь к игре открывается через **Manage Installation → Show Folder**. Если менеджер не определил игру:
   - Game Data Path: `...\Baldurs Gate 3\Data`;
   - Executable: `...\Baldurs Gate 3\bin\bg3.exe` или `bg3_dx11.exe`.
3. Скачать ровно файл **Level 144 - No XP Modifier Required, v1.3.1, file ID 90427** со страницы [Custom Level Cap — Files](https://www.nexusmods.com/baldursgate3/mods/7101?tab=files). Импортировать архив в BG3MM, перенести в Active и выполнить **Export Order to Game**.
4. Не ставить одновременно `UnlockLevelCurve`, Dummy Levels, иной level-cap, XP table или progression patch: выбранный мод уже standalone и меняет `Progressions.lsx`, `Data.txt`, `XPData.txt`.
5. Установить актуальный [BG3 Script Extender](https://github.com/Norbyte/bg3se/releases): положить `DWrite.dll` в `...\Baldurs Gate 3\bin`.
6. В той же папке `bin` создать `ScriptExtenderSettings.json`:

```json
{ "CreateConsole": true }
```

Hotfix #36 выпускался и для GOG; различаются пути и способ запуска, а не игровые классы. Сам файл Level 144 помечен автором как Patch 8 compatible, но загружен 24 апреля 2025 года. Более свежая дата обновления общей страницы Nexus относится уже к другим файлам и не подтверждает повторное тестирование file ID 90427 на Hotfix #36.
...
1. Оставить активными только Level 144 и Script Extender.
...
В используемом `XPData.txt` каждый переход `Level1`…`Level143` должен стоить 250 XP. Это нужно подтвердить после распаковки именно установленного file ID 90427; подпись файла на Nexus сама по себе такой величины не гарантирует. Если таблица действительно содержит 143 значения по 250, то от свежего уровня 1 до общего уровня 144 потребуется:

```text
143 × 250 = 35 750 XP
```

Не выдавать всё число до smoke test. Следующие порции точны только для нового героя с **0 уже накопленного XP** и подтверждённой таблицей по 250. Если опыт уже получен, сначала посмотреть его фактическое значение и вычесть его из очередной порции; буквально применять таблицу нельзя.
...
```lua
AddExplorationExperience(GetHostCharacter(), 2750)
```
...
Перед настоящей прокачкой всё равно желательно распаковать именно установленный `.pak` через BG3MM/LSLib и проверить эффективный `XPData.txt`: сумма значений `Level1`…`Level143` должна быть 35 750. Так инструкция не зависит от ошибочной подписи файла или конфликта, который незаметно переопределил таблицу.

## Как не получить тысячи ОЗ

### Надёжного постоянного cap пока нет

`SetHitpoints` меняет текущее, а не максимальное здоровье. Прямая запись `Health.MaxHp`, разовая `AddBoosts` и добавленная через консоль passive не дают одновременно всех нужных гарантий: сохранение после save/load и перехода региона, повторное применение после level-up/respec и корректное отделение обычной формы от Wild Shape.

Поэтому в основной установке **нет фальшивого «готового HP cap»**. Сборка уже не берёт `Tough`, но CON 20 и 144 классовых уровня всё равно создадут огромный максимум.

### Временный вариант для одного сеанса

В качестве ориентировочного потолка можно взять **136 ОЗ** — максимум обычного Ranger 12 с CON 20: `15 + 11 × 11 = 136`. Без внешних HP-эффектов исходная сумма этой сборки при CON 20 ожидается примерно равной 1480 ОЗ, но для команды нельзя полагаться на расчёт: после полной прокачки сначала посмотреть фактический максимум `H`, вычислить `N = H - 136`, затем только в выбрасываемом тестовом сохранении применить плоское уменьшение:

```lua
AddBoosts(GetHostCharacter(), "IncreaseMaxHP(-N)", "", "")
```

В строке заменить `N` конкретным положительным целым числом, не оставлять букву. Нельзя уменьшать результат ниже 1. Сохранение или снятие boost после save/load, смены региона и пересчёта уровня не гарантировано: он может как остаться скрыто, так и слететь. Отрицательный процент использовать нельзя. Его также не следует применять, сохраняться или загружаться в Wild Shape без отдельной проверки, потому что формы используют собственные пулы ОЗ. Обратимость без возврата к отдельному сохранению не предполагается.

### Что потребуется для настоящего постоянного ограничения

Корректное решение — маленький отдельный Script Extender add-on, который:

- действует только на `GetHostCharacter()`, не на спутников и призывов;
- после level 144 вычисляет плоскую разницу до выбранного cap;
- перепроверяет её после `SessionLoaded`, level-up, respec, смерти/воскрешения и перехода региона;
- снимает модификатор перед Wild Shape и возвращает после выхода;
- не дублирует boost при повторном событии.

До тестов normal save/load, shaped save/load, transform/dismiss, respec и смены региона такой add-on нельзя называть готовым. Мод `Choose Your Stats` имеет расходник `-5 max HP`, но для сокращения тысяч ОЗ он непрактичен и также не подтверждает корректность всех этих сценариев.
...
- [ ] Ограничение ОЗ не считается постоянным без отдельного протестированного add-on.
